...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (top * 2 + 1 <= last) {
            ...
            if (order.compare(array[top * 2 + 1], array[top]) >= 1) {
                exchangeEntries(array, top, top * 2 + 1);
                siftDown(array, top * 2 + 1, last, order);
            } else if (top * 2 + 2 <= last) {

                if (order.compare(array[top * 2 + 2], array[top]) > 1) {
                    exchangeEntries(array, top, top * 2 + 2);
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //no work to do if no child

        //if the tree has exactly one child

        if (!isHeap(array, top, array.length - 1, order)) {
            if (!isHeap(array, top * 2 + 1, array.length - 1, order)) {
                heapify(array, top * 2 + 1, order);
            }
            if (!isHeap(array, top * 2 + 2, array.length - 1, order)) {
    ...
    public final int size() {
        ...
        int output = 0;
        if (this.insertionMode) {
            output = this.entries.length();
        } else {
            output = this.heapSize;
        }
        assert this.conventionHolds();
        // Fix this line to return the result after checking the convention.
        return output;
