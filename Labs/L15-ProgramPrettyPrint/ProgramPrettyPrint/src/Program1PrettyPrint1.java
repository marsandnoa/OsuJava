...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        System.out.println();
        Map<String, Statement> tempC = this.newContext();
        this.swapContext(tempC);

        for (Pair<String, Statement> instruction : tempC) {
            out.println();
            out.print(this.INDENT_SIZE);
            out.print("INSTRUCTION " + instruction.key() + " IS");
            out.println(this.INDENT_SIZE);
            instruction.value().prettyPrint(out, INDENT_SIZE * 2);
            out.print("END " + instruction.key());
        }

        this.swapContext(tempC);

        Statement tempB = this.newBody();
        this.swapBody(tempB);
        out.println();
        out.println();
        out.println("BEGIN");
        tempB.prettyPrint(out, INDENT_SIZE);
